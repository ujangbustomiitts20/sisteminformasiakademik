...
<div class="row mb-4">
    ...
    <div class="col-md-3">
        <div class="card bg-info text-white">
            <div class="card-body text-center">
                <h6>Total Jam Disetujui</h6>
                <h3>{{ number_format($stats['total_jam_disetujui'] ?? 0, 1) }}</h3>
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-3">
    <div class="card-body">
        <div class="row align-items-center">
            ...
            <div class="col-md-6">
                <form action="{{ route('dosen.lembur.index') }}" method="GET" class="d-flex gap-2 justify-content-end">
                    ...
                    @if(request('status'))
                    <a href="{{ route('dosen.lembur.index') }}" class="btn btn-outline-secondary btn-sm">
                ...
                <tbody>
                    @forelse($lemburList as $i => $lembur)
                        <tr>
                            ...
                            <td>{{ Str::limit($lembur->alasan, 30) }}</td>
                            <td>
                                @if($lembur->status === 'disetujui')
                                    <span class="text-success fw-bold">{{ format_rupiah($lembur->total_bayar ?? 0) }}</span>
                                @else
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('dosen.lembur.show', $lembur) }}" class="btn btn-info" title="Detail">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    @if($lembur->status === 'diajukan')
                                    <form action="{{ route('dosen.lembur.destroy', $lembur) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                ...
                                <p class="text-muted mt-2">Belum ada pengajuan lembur</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade" id="modalTambah" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('dosen.lembur.store') }}" method="POST">
                @csrf
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        Pengajuan lembur akan diteruskan ke Kaprodi untuk disetujui, kemudian ke Admin untuk penentuan tarif dan approval final.
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="alasan" class="form-control" rows="3" required placeholder="Jelaskan alasan lembur (min. 10 karakter)">{{ old('alasan') }}</textarea>
                        </div>
                        <div class="col-12 mb-3">
                            <label class="form-label">Pekerjaan yang Dilakukan</label>
                            <textarea name="pekerjaan_yang_dilakukan" class="form-control" rows="3" placeholder="Rincian pekerjaan yang dilakukan selama lembur (opsional)">{{ old('pekerjaan_yang_dilakukan') }}</textarea>
                        </div>
                    </div>
                </div>
