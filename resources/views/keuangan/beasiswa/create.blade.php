...
<div class="container-fluid">
    <div class="mb-4">
        <h1 class="h3 mb-0">Tambah Beasiswa</h1>
        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0"><i class="bi bi-plus-circle me-2"></i>Form Beasiswa Baru</h5>
                </div>
                <div class="card-body">
                    @if(session('error'))
                    ...
                    <form action="{{ route('beasiswa.store') }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="kode" class="form-control @error('kode') is-invalid @enderror" value="{{ old('kode') }}" required placeholder="Contoh: BEA-001">
                                @error('kode')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="sumber_dana" class="form-control @error('sumber_dana') is-invalid @enderror" value="{{ old('sumber_dana') }}" placeholder="Contoh: Pemerintah, Internal, dll">
                                @error('sumber_dana')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select name="tipe_potongan" class="form-select @error('tipe_potongan') is-invalid @enderror" required>
                                    @foreach(\App\Models\Beasiswa::TIPE_POTONGAN as $key => $label)
                                        <option value="{{ $key }}" {{ old('tipe_potongan') == $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('tipe_potongan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="nilai_potongan" class="form-control @error('nilai_potongan') is-invalid @enderror" value="{{ old('nilai_potongan') }}" min="0" step="0.01" required>
                                @error('nilai_potongan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">Jika persen, masukkan angka 0-100</small>
                            </div>
                        </div>
