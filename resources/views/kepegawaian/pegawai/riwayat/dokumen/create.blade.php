...
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('kepegawaian.pegawai.riwayat.dokumen.store', $pegawai) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select name="jenis_dokumen" id="jenis_dokumen" class="form-select @error('jenis_dokumen') is-invalid @enderror" required>
                                    ...
                                    <optgroup label="Dokumen SK">
                                        <option value="SK_CPNS" {{ old('jenis_dokumen') == 'SK_CPNS' ? 'selected' : '' }}>SK CPNS</option>
                                        <option value="SK_PNS" {{ old('jenis_dokumen') == 'SK_PNS' ? 'selected' : '' }}>SK PNS</option>
                                        <option value="SK_Pengangkatan" {{ old('jenis_dokumen') == 'SK_Pengangkatan' ? 'selected' : '' }}>SK Pengangkatan</option>
                                        <option value="SK_Kenaikan_Pangkat" {{ old('jenis_dokumen') == 'SK_Kenaikan_Pangkat' ? 'selected' : '' }}>SK Kenaikan Pangkat</option>
                                        <option value="SK_Jabatan" {{ old('jenis_dokumen') == 'SK_Jabatan' ? 'selected' : '' }}>SK Jabatan</option>
                                    </optgroup>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" name="nomor_dokumen" id="nomor_dokumen" class="form-control @error('nomor_dokumen') is-invalid @enderror" value="{{ old('nomor_dokumen') }}">
                                @error('nomor_dokumen')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
