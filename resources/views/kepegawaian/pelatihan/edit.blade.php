...
@section('title', 'Edit Riwayat Pelatihan - ' . $dosen->nama)
...
<div class="card">
    <div class="card-header">
        <i class="bi bi-journal-bookmark me-2"></i>Edit Riwayat Pelatihan/Sertifikasi - {{ $dosen->nama }}
    </div>
    <div class="card-body">
        <form action="{{ route('kepegawaian.pelatihan.update', [$dosen, $pelatihan]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select name="jenis" id="jenis" class="form-select @error('jenis') is-invalid @enderror" required>
                            ...
                            <option value="Diklat" {{ old('jenis', $pelatihan->jenis) == 'Diklat' ? 'selected' : '' }}>Diklat (Pendidikan & Pelatihan)</option>
                            ...
                            <option value="Kursus" {{ old('jenis', $pelatihan->jenis) == 'Kursus' ? 'selected' : '' }}>Kursus</option>
                            <option value="Lainnya" {{ old('jenis', $pelatihan->jenis) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('jenis')
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" name="penyelenggara" id="penyelenggara" class="form-control @error('penyelenggara') is-invalid @enderror" value="{{ old('penyelenggara', $pelatihan->penyelenggara) }}" required>
                        @error('penyelenggara')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="form-control @error('tanggal_mulai') is-invalid @enderror" value="{{ old('tanggal_mulai', $pelatihan->tanggal_mulai ? $pelatihan->tanggal_mulai->format('Y-m-d') : '') }}" required>
                        @error('tanggal_mulai')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control @error('tanggal_selesai') is-invalid @enderror" value="{{ old('tanggal_selesai', $pelatihan->tanggal_selesai ? $pelatihan->tanggal_selesai->format('Y-m-d') : '') }}" required>
            ...
            <div class="mb-3">
                ...
                <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror" rows="3">{{ old('keterangan', $pelatihan->keterangan) }}</textarea>
                @error('keterangan')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
