...
<div class="card">
    <div class="card-header">
        <i class="bi bi-plus-lg me-2"></i>Form Tambah Jadwal Kuliah
    </div>
    <div class="card-body">
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="mata_kuliah_id" id="mata_kuliah_id" class="form-select @error('mata_kuliah_id') is-invalid @enderror" required>
                            ...
                            <option value="{{ $mk->id }}" {{ old('mata_kuliah_id') == $mk->id ? 'selected' : '' }}>{{ $mk->kode }} - {{ $mk->nama }} ({{ $mk->sks }} SKS)</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="dosen_id" id="dosen_id" class="form-select @error('dosen_id') is-invalid @enderror" required>
                            ...
                            @foreach($dosen as $d)
                            <option value="{{ $d->id }}" {{ old('dosen_id') == $d->id ? 'selected' : '' }}>{{ $d->nidn }} - {{ $d->nama }}</option>
                            @endforeach
                        </select>
                        @error('dosen_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="ruangan_id" id="ruangan_id" class="form-select @error('ruangan_id') is-invalid @enderror" required>
                            ...
                            <option value="{{ $r->id }}" {{ old('ruangan_id') == $r->id ? 'selected' : '' }}>{{ $r->kode }} - {{ $r->nama }} (Kap: {{ $r->kapasitas }})</option>
                            @endforeach
                        </select>
                        @error('ruangan_id')
                        <div class="invalid-feedback">{{ $message }}</div>
