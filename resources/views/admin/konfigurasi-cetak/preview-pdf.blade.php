...
@section('title', 'Preview - ' . $config->nama)
...
@section('content')
    ...
    @else
        @include('cetak.components.header-simple', ['title' => $config->judul ?? $config->nama])
    @endif

    <div class="doc-title">
        <h3>{{ $config->judul ?? $config->nama }}</h3>
    ...
    <table class="info-table">
        <tr>
            ...
            <td><strong>{{ $mahasiswa->nama ?? 'NAMA MAHASISWA CONTOH' }}</strong></td>
        </tr>
        <tr>
            <td>NIM</td>
            <td>:</td>
            <td>{{ $mahasiswa->nim ?? '2024001001' }}</td>
        </tr>
        @if(isset($mahasiswa->programStudi))
        <tr>
            <td>Program Studi</td>
            <td>:</td>
        ...
        @endif
        @if(isset($mahasiswa->programStudi->fakultas))
        <tr>
            <td>Fakultas</td>
            <td>:</td>
    ...
    @if($config->tampilkan_ttd)
        @include('cetak.components.signature', [
            'jabatan' => $config->jabatan_ttd ?? 'Pejabat Berwenang',
