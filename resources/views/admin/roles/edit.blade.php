...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">Edit Role: {{ $role->nama }}</h1>
        ...
        <div class="row">
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="slug" class="form-control @error('slug') is-invalid @enderror" 
                                   value="{{ old('slug', $role->slug) }}" {{ $role->is_system ? 'readonly' : '' }}>
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            @if($role->is_system)
                                <small class="text-warning">Slug role sistem tidak dapat diubah</small>
                            @endif
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="warna" class="form-select">
                                @foreach($warna as $key => $label)
                                    <option value="{{ $key }}" {{ old('warna', $role->warna) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="mt-2">
                                Preview: <span class="badge bg-{{ $role->warna }}">{{ $role->nama }}</span>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Urutan</label>
                            <input type="number" name="urutan" class="form-control" value="{{ old('urutan', $role->urutan) }}" min="0">
                        </div>
                        ...
                        @if($role->is_system)
                            <div class="alert alert-warning mt-3 mb-0">
                                ...
                                Ini adalah role sistem. Beberapa pengaturan tidak dapat diubah.
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="d-grid gap-2">
                    ...
                    <a href="{{ route('admin.roles.duplicate', $role) }}" class="btn btn-outline-info">
                        <i class="bi bi-copy me-2"></i>Duplikasi Role Ini
                    </a>
                </div>
            </div>
            ...
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm mb-4 border-primary" style="border-left: 4px solid #0d6efd !important;">
                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">
                            <i class="bi bi-list me-2"></i>Akses Menu
                            <span class="badge bg-warning text-dark ms-1">{{ count($roleMenus) }}</span>
                        </h5>
                    ...
                    <div class="card-body" style="max-height: 600px; overflow-y: auto;">
                        <div class="alert alert-info small mb-3">
                            ...
                            <strong>Pilih menu = Otomatis dapat akses halaman + permission</strong>
                        </div>
                            ...
                            <div class="mb-2">
                                <div class="form-check">
                                    <input type="checkbox" name="menus[]" value="{{ $menu->id }}" 
                                           ...
                                           data-parent="{{ $menu->id }}"
                                           {{ in_array($menu->id, old('menus', $roleMenus)) ? 'checked' : '' }}>
                                        ...
                                        @foreach($menu->children as $child)
                                            <div class="form-check">
                                                <input type="checkbox" name="menus[]" value="{{ $child->id }}" 
                                                       ...
                                                       data-parent="{{ $menu->id }}"
                                                       {{ in_array($child->id, old('menus', $roleMenus)) ? 'checked' : '' }}>
                        ...
                        <a class="text-decoration-none d-flex justify-content-between align-items-center" 
                           data-bs-toggle="collapse" href="#permissionSection" role="button" aria-expanded="false">
                            <h5 class="card-title mb-0 text-secondary">
                                ...
                                <span class="badge bg-secondary ms-1">{{ count($rolePermissions) }}</span>
                            </h5>
                    ...
                    <div class="collapse" id="permissionSection">
                        <div class="card-body border-top" style="max-height: 600px; overflow-y: auto;">
                            <div class="alert alert-warning small mb-3">
                                ...
                                Permission sudah otomatis dari menu yang dipilih. Gunakan ini hanya jika ingin membatasi aksi tertentu (misal: boleh lihat tapi tidak boleh hapus).
                            </div>
